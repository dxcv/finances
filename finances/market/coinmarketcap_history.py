...
def extract_data(html):
  ...
  for row in raw_rows:
    ...
    row = [ field.translate(translation_table) for field in row ]
    rows.append(row)
...
def processDataFrame(df):
  ...
  for col in cols:
    df.loc[:,col] = df[col].apply(conver_string_to_float)
...
def main(args=None):
  ...
  if(args.dataframe):
    import pandas as pd
    return processDataFrame(pd.DataFrame(data=rows,columns=header)).set_index('Date')
  else:  
